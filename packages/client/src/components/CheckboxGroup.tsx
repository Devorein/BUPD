...
export function CheckboxGroup<State>(props: CheckboxGroupProps<State>) {
	...
	return (
		<div className="CheckboxGroup flex flex-col gap-3">
			<Typography variant="h5">{label}</Typography>
			<div className="flex flex-col gap-3">
				{items.map((item) => {
					const currentLabel = Array.isArray(item) ? item[1] : item,
					...
					return (
						<div className="flex items-center" key={value}>
							...
							/>
							<Typography component="div" className="flex-grow capitalize">
								{currentLabel}
							</Typography>
						</div>
					);
