...
export function Paginate<ClientQuery extends IQuery<any, any>, Sort extends [string, -1 | 1], Data>(
	props: PaginateProps<ClientQuery, Data>
) {
	const {
	...
	const [dummyQuery, setDummyQuery] = useState<ClientQuery>(clientQuery);

	const { hasNextPage, lastFetchedPage, fetchNextPage, data, totalItems, allItems, isFetching } =
		dataFetcher(clientQuery);

	if (data) {
		return (
			<div className={`flex justify-center gap-5 py-5 w-full h-full ${className}`}>
				{filterGroups.length !== 0 && (
					<div className="h-full min-w-[300px]">
						<FilterForm<ClientQuery>
							clientFilter={dummyQuery.filter}
							...
							resetFilter={() => clientQueryFn().filter}
						/>
					</div>
				)}

				<div className="flex gap-2 flex-col w-full h-full">
					<Typography className="mb-5 uppercase" variant="h4">
						{label}
					</Typography>
					<div
					...
					>
						<div className="flex justify-between items-center">
							<div className="flex gap-3 w-full">
								<Select<number>
									value={parseInt(clientQuery.limit?.toString() ?? '5', 10)}
									items={[5, 10, 15, 20, 25]}
								...
								/>
								<Select<string>
									value={clientQuery.sort.join('.')}
									items={Object.keys(sortLabelRecord)}
									menuItemRender={(value) => sortLabelRecord[value]}
									renderValue={(value) => sortLabelRecord[value]}
									onChange={(event) => {
										setClientQuery({
											...clientQuery,
								...
								{searchBarPlaceholder && (
									<SearchBar
										value={clientQuery.filter.search?.join(' ') ?? ''}
					...
					<div style={{ marginTop: 15 }}>
						<LoadMoreButton
							...
							hasNextPage={hasNextPage && allItems.length !== totalItems}
						/>
					</div>
				</div>
			</div>
