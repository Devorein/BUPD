...
function DashboardItem(props: DashboardItemProps) {
	...
	return (
		<div
			className={`shadow-md border-2 p-5 rounded-md flex flex-col justify-between gap-3 ${className}`}
		>
			<div className="flex flex-col gap-2">
				...
				{children && <div className="flex gap-10">{children}</div>}
			</div>
			<Button
				style={{
					width: 'fit-content',
				}}
				content={`View ${label.toLowerCase()}`}
				onClick={() => router.push({ pathname: pathname ?? `/${label.toLowerCase()}` })}
			/>
		</div>
	);
}
...
export function Dashboard() {
	useIsAuthenticated();
	const { data: getDashboardQueryData, isLoading: isGetDashboardQueryLoading } =
		useGetDashboardQuery();
	const dashboardData =
		getDashboardQueryData?.status === 'success' ? getDashboardQueryData.data : null;
	...
	return dashboardData && !isGetDashboardQueryLoading ? (
			...
			<div className="flex gap-3 flex-col">
				<DashboardItem label="Casefiles" total={totalCasefiles} className="flex-grow">
					...
					<div className="flex flex-col gap-1">
						...
						<div>
							<DetailsList
								items={Object.entries(dashboardData.casefiles.priority).map(([priority, total]) => [
			...
			<DashboardItem label="Accesses" total={totalAccesses} className="min-w-[250px]">
				<div className="flex flex-col gap-5 flex-grow">
					<div className="flex gap-2 flex-col">
						...
						<DetailsList
							items={Object.entries(dashboardData.accesses.approval).map(([key, value]) => [
								APPROVAL_RECORD[key.toString()]!,
								value,
							])}
						/>
					</div>
