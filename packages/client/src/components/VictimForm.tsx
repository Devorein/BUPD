			...
			{({ isSubmitting, isValid }) => (
				<Form className={`p-5 shadow-md rounded-md h-full border-2 w-full`}>
					...
					>
						...
						<FormikTextInput name={`address`} label={`Address`} placeholder={`123 Road`} />
						<FormikTextInput type="number" name={`age`} label={`Age`} placeholder={'25'} />
						<FormikTextInput name={`phone_no`} label={`Phone number`} placeholder={`+8801...`} />
						<FormikTextInput
							multiline
							rows={5}
							name={`description`}
