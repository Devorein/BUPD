...
import {
	...
	useUpdateCriminalMutationCache,
} from '../api/mutations/useUpdateCriminalMutation';
				...
				{({ openModal }) => (
					<>
						<TransitionedModal isModalOpen={isUpdateModalOpen} onClose={closeUpdateModal}>
							<CriminalForm<UpdateCriminalPayload>
								initialValues={selectedUpdateData!}
								isMutationLoading={updateCriminalMutation.isLoading}
								onSubmit={(values) => {
									if (selectedUpdateData) {
										updateCriminalMutation.mutate(
											...
											updateCriminalMutationCache(selectedUpdateData.criminal_id, () =>
												closeUpdateModal()
											)
										);
									}
								...
								<div className="grid grid-cols-5 gap-5 pr-5">
									{criminals.map((criminal) => {
										...
										if (currentUser.type === 'admin') {
											...
											mutateIcons = (
												<MutateIcons
													showDeleteIcon={criminal.permissions?.delete === 1}
													showUpdateIcon={criminal.permissions?.update === 1}
													onDeleteIconClick={() => {
														openModal(criminal);
													}}
													onUpdateIconClick={() => {
										...
										if (currentUser.type === 'police') {
											permissionIcons = (
												<div className="flex gap-1">
													{criminal.permissions?.read ? (
														<VisibilityOutlinedIcon style={{ fill: grey[500] }} fontSize="small" />
													) : (
															...
															onClick={() => {
																createAccessMutation.mutate(
																	...
																	createAccessMutationCache(
																		...
																		`Successfully sent read request for criminal ${criminal.criminal_id}`
																	)
																);
															}}
														...
														<EditOutlinedIcon
															onClick={() => {
																createAccessMutation.mutate(
																	...
																	createAccessMutationCache(
																		criminal.criminal_id,
																		`Successfully sent update request for criminal ${criminal.criminal_id}`
																	)
																);
															}}
															className="cursor-pointer"
														...
														<DeleteOutlinedIcon
															onClick={() => {
																createAccessMutation.mutate(
																	...
																	createAccessMutationCache(
																		criminal.criminal_id,
																		`Successfully sent delete request for criminal ${criminal.criminal_id}`
																	)
																);
															}}
															className="cursor-pointer"
